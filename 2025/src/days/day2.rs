...
impl Day for Day2 {
    ...
    fn part1(data: &Vec<(i64, i64)>) -> String {        
        let total = iterate_ranges(data,
            |str| str[.. str.len()/2] == str[str.len()/2 ..]
        );
        
        return highlight_part!("Ids that repeat twice: {}", total);
    }

    fn part2(data: &Vec<(i64, i64)>) -> String {
        ...
        return highlight_part!("Ids that repeat multiple times: {}", total);
    }
}